...
async function init() {
    ...
    if(platform === "darwin") {
        ...
        new window.windowbar({'style':'win', 'dblClickable':false, 'fixed':true, 'title':document.title,'dark':true})
            .appendTo(document.body)
        $('.windowbar').prepend("<img src='img/icon-titlebar-dark.png' alt='ícone' class='windowbar-icon'>")
    ...
    // Atualiza o placeholder para um link copiado
    ...
    // Verifica a URL quando o atalho de adicionar é usado
    ...
    // Inicializa os cartões arrastáveis
    ...
    // Inicializa o menu suspenso "Quando terminar"
    ...
    //Initialize select2
    $("#subsLang").select2({width: '75%', placeholder: "Selecione legenda", language: {noResults: () => "Nenhuma legenda encontrada"}});
    $("#autoGenSubsLang").select2({width: '75%', placeholder: "Selecione legendas auto-geradas", language: {noResults: () => "Nenhuma legenda encontrada"}} );
    $("#sponsorblockMark").select2({width: '75%', placeholder: "Selecione seções de um vídeo para marcar"});
    $("#sponsorblockRemove").select2({width: '75%', placeholder: "Selecione seções de um vídeo para remover"});
    ...
    $('#downloadBtn').on('click', async () => {
        ...
        for(const card of videoCards) {
            ...
            if(isDownloadable) {
                ...
                if($(card).hasClass("unified")) {
                    ...
                    $(card).find('.metadata.info').html('Baixando playlist...');
                    $(card).find('select').addClass("d-none");
                }
            }
        }
        ...
        $('#totalProgress .progress-bar').remove();
        $('#totalProgress').prepend('<div class="progress-bar" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>')
        $('#totalProgress small').html(`Baixando - item 0 de ${videos.length} completado(s)`);
    });
    ...
    window.main.receive("binaryLock", (args) => {
        if(args.lock === true) {
            ...
            $('#add-url').attr("placeholder", "Insira uma URL de vídeo/playlist para adicionar à fila").prop("disabled", false);
        }
    })
...
async function addVideo(args) {
    ...
    if(args.type === "single") {
        ...
        $(template).find('.progress small').html("Configurando o ambiente")
        $(template).find('.metadata.left').html('<strong>Duração: </strong>' + ((args.duration == null) ? "Unknown" : args.duration));
        ...
        if(!args.hasFilesizes) {
            $(template).find('.metadata.right').html('<strong>Tamanho: </strong>Desconhecido');
        } else if(args.loadSize) {
            $(template).find('.metadata.right').html('<strong>Tamanho: </strong><i class="lds-dual-ring"></i>');
        } else {
            $(template).find('.metadata.right').html('<strong>Tamanho: </strong><button class="btn btn-dark">Calcular</button>')
        }
    ...
    $(template).find('.download-btn').on('click', () => {
        ...
        $(template).find('.metadata.info').html('Pesquisando metadados...');
...
async function setUnifiedPlaylist(args) {
    ...
    $(card).find('.metadata.right').html('<strong>Tamanho playlist: </strong>' + args.length);
    $(card).find('.metadata.left').html('<strong>Canal: </strong>' + (args.uploader == null ? "Desconhecido" : args.uploader));
    if(window.settings.enableEncoding) {
        $(card).find('.metadata').hide();
    } else {
        $(card).find('.custom-select.download-encoding, .custom-select.download-audio-encoding').hide();
    }
    if(args.subtitles) $(card).find('.subtitle-btn i').removeClass("bi-card-text-strike").addClass("bi-card-text").attr("titulo", "Legendas ativadas");
    ...
    $(card).find('.progress-bar')
        ...
        .width("100%")
        .prop("aria-valuenow", "indefinite");
    $(card).find('.progress small').html('Configurando ambiente');
...
function updateProgress(args) {
    ...
    if(args.progress.finished != null && args.progress.finished) {
        if(args.progress.isPlaylist) {
            $(card).find('.progress small').html("Playlist baixada - 100%");
            ...
            $(card).find('.open .folder').html("Mostrar arquivos na pasta");
            $(card).find('.open').addClass("d-flex");
        } else {
            if(args.progress.isAudio == null) $(card).find('.progress small').html("Item baixado - 100%");
            else $(card).find('.progress small').html((args.progress.isAudio ? "Audio" : "Video") + " baixado - 100%");
            $(card).find('.progress-bar').attr('aria-valuenow', 100).css('width', "100%");
            $(card).find('.options').addClass("d-none").removeClass("d-flex");
            $(card).find('.progress-bar').removeClass("progress-bar-striped")
            $(card).find('.open').addClass("d-flex");
    ...
    if(args.progress.done != null && args.progress.total != null) {
        ...
        if(args.progress.percentage === "100.0%") {
            ...
            $(card).find('.progress small').html("Convertendo com FFmpeg");
        } else {
            if (args.progress.isAudio == null) $(card).find('.progress small').html("Baixando item - " + args.progress.percentage);
            else $(card).find('.progress small').html((args.progress.isAudio ? "Baixando audio" : "Baixando video") + " - " + args.progress.percentage);
        }
        if(!progressCooldown.includes(args.identifier)) {
            ...
            $(card).find('.metadata.left').html('<strong>Velocidade: </strong>' + args.progress.speed).show();
            setTimeout(() => {
...
function updateTotalProgress(args) {
    ...
    $('#totalProgress small').html(`Baixando - item ${args.progress.done} de ${args.progress.total} finalizado(s)`);
...
function updateSize(identifier, clicked) {
    ...
    if($(card).find('.custom-select.download-quality').prop("disabled") === true) {
        sizeCooldown = sizeCooldown.filter(item => item !== identifier);
        $(card).find('.metadata.right').html('<strong>Tamanho: </strong>' + "Desconhecido");
        return;
    }
    const formatLabel = $(card).find('.custom-select.download-quality').val();
    $(card).find('.metadata.right').html('<strong>Tamanho: </strong><i class="lds-dual-ring"></i>');
    window.main.invoke("videoAction", {
        ...
        if(size != null && size === "Unknown") {
            $(card).find('.metadata.right').html('<strong>Tamanho: </strong>' + "Unknown");
        } else if(size != null) {
            if($(card).find('.custom-select.download-quality').val() === formatLabel) {
                ...
                $(card).find('.metadata.right').html('<strong>Tamanho: </strong>' + convertBytes(size));
            }
        } else {
            $(card).find('.metadata.right').html('<strong>Tamanho: </strong><button class="btn btn-dark">Calcular</button>');
        }
        sizeCooldown = sizeCooldown.filter(item => item !== identifier)
        updateTotalSize();
    });
...
async function updateTotalSize() {
    ...
    if(total > 0) $('#totalProgress small').html('Pronto pro download! - Tamanho total da fila: ' + convertBytes(total));
    else  $('#totalProgress small').html('Pronto para baixar!');
}
...
function saveSubtitlesModal() {
    ...
    if($(modal).find('#enableSubs').is(":checked")) $(card).find('.subtitle-btn i').removeClass("bi-card-text-strike").addClass("bi-card-text").attr("title", "Legendas ativadas");
    else $(card).find('.subtitle-btn i').removeClass("bi-card-text").addClass("bi-card-text-strike").attr("title", "Legendas desativadas");
...
function showInfoModal(info, identifier) {
    ...
    if(data == null) {
        ...
        data = {
            ...
            description: "Este vídeo apresentou um erro. Nem todas as informações estão disponíveis.\n\nError:\n" + $(card).find('.metadata').text(),
            url: $(card).find('.url').val()
        }
    }
    $(modal).find('img').prop("src", data.thumbnail);
    $(modal).find('.modal-title').html(data.title);
    $(modal).find('#info-description').html(data.description == null ? "Nenhuma descrição encontrada." : data.description);
    $(modal).find('.uploader').html('<strong>Canal: </strong>' + (data.uploader == null ? "Unknown" : data.uploader));
    $(modal).find('.extractor').html('<strong>Plataforma: </strong>' + (data.extractor == null ? "Unknown" : data.extractor));
    ...
    $(modal).find('[title="Avaliação"]').html('<i class="bi bi-star"></i> ' + (data.average_rating == null ? "-" : data.average_rating.toString().slice(0,3)));
    $(modal).find('[title="Duração"]').html('<i class="bi bi-clock"></i> ' + (data.duration == null ? "-" : data.duration));
...
function resetTotalProgress() {
    $('#totalProgress .progress-bar').remove();
    $('#totalProgress').prepend('<div class="progress-bar" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>')
    $('#totalProgress small').html('Pronto para baixar!');
    window.main.invoke("iconProgress", -1);
}
...
function changeDownloadIconToLog(card) {
    ...
    $(card).find('.download-btn i')
        ...
        .on('click', () => {
            ...
            $('#logModal .log').html("Carregando log...");
            openLog(id);
...
function openLog(identifier) {
    ...
    window.main.invoke("getLog", identifier).then(log => {
        if(log == null) {
            $(logBox).val("Nenhum registro foi encontrado para este vídeo.")
        } else {
            let fullLog = "";
