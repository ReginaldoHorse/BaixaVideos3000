...
function startCriticalHandlers(env) {
    ...
    if(env.settings.updateBinary) {
        ...
        win.webContents.send("binaryLock", {lock: true, placeholder: `Verificando a nova versão do ffmpeg...`})
        ffmpegUpdater.checkUpdate().finally(() => {
            win.webContents.send("binaryLock", {lock: true, placeholder: `Procurando por atualizações do yt-dlp...`})
