...
class Video {
    ...
    setFilename(liveData) {
        if(liveData.includes("[download] Destination: ")) {
            const replaced = liveData.replace("[download] Destino: ", "");
            this.filename = path.basename(replaced);
        } else if(liveData.includes("[ffmpeg] Juntando formato em \"")) {
            const noPrefix = liveData.replace("[ffmpeg] Juntando formatos em \"", "");
            this.filename = path.basename(noPrefix.trim().slice(0, -1));
        } else if(liveData.includes("[ffmpeg] Adding metadata to '")) {
            const noPrefix = liveData.replace("[ffmpeg] Adicionando metadados para '", "");
            this.filename = path.basename(noPrefix.trim().slice(0, -1));
        } else if(liveData.includes("[Merger] Merging formats into \"")) {
            const noPrefix = liveData.replace("[Merger] Juntando formatos em \"", "");
            this.filename = path.basename(noPrefix.trim().slice(0, -1));
        } else if(liveData.includes("[ExtractAudio] Destination: ")) {
            const replaced = liveData.replace("[ExtractAudio] Destino: ", "");
            this.filename = path.basename(replaced);
